...
from src.ragforge.config.prompts import RAG_SYSTEM_PROMPT
...
def resolve_originals(docs: List[Document]) -> List[Document]:
    ...
    but the LLM should reason over the original content.
...
def build_rag_chain(
    retriever: Any, question: str, history: List[Dict[str, Any]]
) -> Any:
    ...
    """
    from src.ragforge.services.llm_service import get_qa_llm
