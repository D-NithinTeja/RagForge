...
def partition_document(
    ...
    logger.info(
        "Initializing document partitioning layout: %s (strategy=%s)",
        file_path,
        strategy,
    )

    if not Path(file_path).is_file():
        raise FileNotFoundError(f"Target ingestion asset file absent at: {file_path}")
    ...
    logger.info(
        "Partitioning phase complete: %d semantic fragments unrolled from %s",
        len(chunks),
        file_path,
    )
    return chunks
